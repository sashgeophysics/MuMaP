...
CONTAINS
  ...
  FUNCTION load_seismo_variable_temp(c,dim,fname,mtzdepth,NORM,&
       &clap,eta,func,depth_in)
    IMPLICIT NONE
    TYPE(composition),INTENT(in)::c
    real(sp),intent(in)::mtzdepth,clap,eta
    ...
    ALLOCATE(DATA(load_seismo_variable_temp%NGRID,11),&
         &x(load_seismo_variable_temp%NGRID,3))
    ...
    DO ii=1,load_seismo_variable_temp%NGRID
       ...
       load_seismo_variable_temp%CELL(ii)%temperature=DBLE(load_seismo_variable_temp%comp&
            &%potential_temperature)+depth2dT(DATA(ii,4),mtzdepth,clap)

       !> Convert the temperature to  Shear wave velocity
       !!using the function temp2vs in module microgeodynamics!<
       load_seismo_variable_temp%CELL(ii)%vs_sol=temp2vs(load_seismo_variable_temp%CELL(ii)&
       ...
       load_seismo_variable_temp%CELL(ii)%vp_sol=temp2vp(load_seismo_variable_temp%CELL(ii)&
            &%temperature,&
       ...
       load_seismo_variable_temp%CELL(ii)%rho=temp2rho(load_seismo_variable_temp%CELL(ii)&
            &%temperature,&
            &load_seismo_variable_temp%comp%basalt_fraction)
       !Calculate the corresponding solid K,G, and nu
       CALL vel2mod(load_seismo_variable_temp%CELL(ii)%vs_sol,load_seismo_variable_temp%CELL(ii)&
            &%vp_sol,&
            &load_seismo_variable_temp%CELL(ii)%rho,load_seismo_variable_temp%CELL(ii)%G_sol,&
            &load_seismo_variable_temp%CELL(ii)%K_sol,load_seismo_variable_temp%CELL(ii)%nu )
       !> Calculate observed Shear wave velocity from impedance
       !!contrast observation!<
       impedance_vel=impedance_velocity_410(1)
       if (NORM==.TRUE.) then
       impedance350=DATA(ii,7)*eta
       else
  ...
  FUNCTION load_seismo_mean_temp(c,dim,fname,mtzdepth,NORM&
       &,clap,eta,depth_in)
    IMPLICIT NONE
    TYPE(composition),INTENT(in)::c
    real(sp),intent(in)::mtzdepth,clap,eta
    ...
    logical,intent(in)::NORM
    REAL(sp),INTENT(in),OPTIONAL::depth_in
    REAL(sp)::z,impedance350,prem_impedance,mean_temp
    REAL(sp),DIMENSION(2)::impedance_vel
    REAL(sp),DIMENSION(3)::prem_value
    REAL(sp),DIMENSION(:,:),POINTER::DATA
    REAL(sp),DIMENSION(:,:),POINTER::x
    TYPE(region)::load_seismo_mean_temp
    INTEGER::ii
    ...
    load_seismo_mean_temp%comp=c
    load_seismo_mean_temp%NGRID=line_numbers(fname,10000)
    load_seismo_mean_temp%MTZ=mtzdepth

    ALLOCATE(DATA(load_seismo_mean_temp%NGRID,11),x(load_seismo_mean_temp%NGRID,3))
    OPEN(10,file=fname,status='unknown',form='formatted')
    DO ii=1,load_seismo_mean_temp%NGRID
       READ(10,'(11(1x,es10.3))'),DATA(ii,:)
    END DO
    CLOSE(10)

    ALLOCATE(load_seismo_mean_temp%CELL(load_seismo_mean_temp%NGRID),&
         &load_seismo_mean_temp%LOC(load_seismo_mean_temp%NGRID,3),&
         &load_seismo_mean_temp%TOPO(load_seismo_mean_temp%NGRID,2))
    mean_temp=0.0_sp
    DO ii=1,load_seismo_mean_temp%NGRID
       mean_temp=mean_temp+depth2dT(DATA(ii,4),mtzdepth,clap)
    END DO
    mean_temp=mean_temp/REAL(load_seismo_mean_temp%NGRID,kind=sp)
    !> Here we load the observed variables into the region !<
    DO ii=1,load_seismo_mean_temp%NGRID
       ! Location in Lat,Lon,Depth(km)
       load_seismo_mean_temp%LOC(ii,1:3)=DATA(ii,1:3)

       !Depth of top of LVL and 410 in (km)
       load_seismo_mean_temp%TOPO(ii,1:2)=DATA(ii,3:4)

       !>calculate potential temperature from depression of 410
       !!Using function depth2dT in module microgeodynamics!<
       load_seismo_mean_temp%CELL(ii)%temperature=DBLE(load_seismo_mean_temp%comp&
            &%potential_temperature)+mean_temp

       !> Convert the temperature to  Shear wave velocity
       !!using the function temp2vs in module microgeodynamics!<
       load_seismo_mean_temp%CELL(ii)%vs_sol=temp2vs(load_seismo_mean_temp%CELL(ii)&
            &%temperature,load_seismo_mean_temp%comp%basalt_fraction)

       !> Convert the temperature to  P wave velocity
       !!using the function temp2vp in module microgeodynamics!<
       load_seismo_mean_temp%CELL(ii)%vp_sol=temp2vp(load_seismo_mean_temp%CELL(ii)&
            &%temperature,&
            &load_seismo_mean_temp%comp%basalt_fraction)

       !> Convert the temperature to density using the function
       !! temp2rho in module microgeodynamics!<
       load_seismo_mean_temp%CELL(ii)%rho=temp2rho(load_seismo_mean_temp%CELL(ii)&
            &%temperature,&
            &load_seismo_mean_temp%comp%basalt_fraction)
       !Calculate the corresponding solid K,G, and nu
       CALL vel2mod(load_seismo_mean_temp%CELL(ii)%vs_sol,load_seismo_mean_temp%CELL(ii)&
            &%vp_sol,&
            &load_seismo_mean_temp%CELL(ii)%rho,load_seismo_mean_temp%CELL(ii)%G_sol,&
            &load_seismo_mean_temp%CELL(ii)%K_sol,load_seismo_mean_temp%CELL(ii)%nu )
       !> Calculate observed Shear wave velocity from impedance
       !!contrast observation!<
       impedance_vel=impedance_velocity_410(1)
       ...
       ! Impedance contrast from the PREM model across 400
       ! Calculate observed velocity from PREM model
       prem_value=PREM_vs_vp_rho(350.0_sp)
       load_seismo_mean_temp%CELL(ii)%vs_obs=prem_value(1)&
            &*(1.0+impedance350)
       !Impedance contrast at 410
       load_seismo_mean_temp%CELL(ii)%vp_obs=1.0+impedance350
    END DO

    DATA=>NULL()
    x=>NULL()
  END FUNCTION load_seismo_mean_temp
  ...
  FUNCTION impedance_velocity_410(int)
    IMPLICIT NONE
    INTEGER,INTENT(in)::int
    REAL(sp),DIMENSION(2)::impedance_velocity_410
    REAL(sp)::vstop,vsbot,rhotop,rhobot
    rhotop=3543.25_sp
    rhobot=3723.78_sp
    IF (int==1) THEN  
    ...
    END IF
    impedance_velocity_410(1)=(rhobot*vsbot-rhotop*vstop)/rhotop/vstop
    impedance_velocity_410(1)=(vsbot-vstop)/vstop
    impedance_velocity_410(2)=vstop
  END FUNCTION impedance_velocity_410
